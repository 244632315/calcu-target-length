...
for step in range(train_times):
    ...
    if step % 100 == 0:
        print(step,sess.run(loss,feed_dict={xs:data_test,ys:y_test}))
    if step == (train_times-1) :
       print(step, sess.run(ys, feed_dict={ys: y_test}))
       print(step, sess.run(prediction, feed_dict={xs: data_test, ys: y_test}))
       print(step, sess.run(loss, feed_dict={xs: data_test, ys: y_test}))
writer.close()
